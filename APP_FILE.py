...
if Side=="Main Page":
    ...
    st.markdown(
        ...
        **👈 Select a demo from the sidebar** to visualize the Results!
        this is the test. This is test 2
        ### Circuit Diagram:
    """
    )
    #image = Image.open('Circuit.png')
    image = Image.open('fault.jfif')
...
################################################################# 3rd selection ###################################################### 
    ...
    p=pd.read_csv('fault_0.1.csv')
    p2=pd.read_csv('0.1_fault_combine.csv')
    ...
    if st.sidebar.checkbox("Display Results", False):
        st.subheader("All Machine Learning Classification Models Results:")
        st.write(p2)
        
    classifier = st.sidebar.selectbox("Choose Classifier Model", ("K Neighbors Classifier","Light Gradient Boosting Machine"))
    st.write("Selected Model: ",classifier)


    if classifier=="Light Gradient Boosting Machine":
        option1 = st.sidebar.radio('Ploting Performance Parameters:',['Confusion Matrix','Error','Model Class_report','Validation Curve'])
        if option1=="Confusion Matrix":
            image = Image.open('LBGM_CM.png')
            st.image(image, caption='')

        elif option1=="Error":
            image = Image.open('LBGM_ERROR.png')
            st.image(image, caption='')

        elif option1=="Model Class_report":
            image = Image.open('LBGM_CR.png')
            st.image(image, caption='')

        elif option1=="Validation Curve":
            image = Image.open('LBGM_VC.png')
            st.image(image, caption='')
...
################################################################# 3rd selection ###################################################### 
    ...
    st.subheader("Shabbir : I'm feeling clumzy")
    image = Image.open('app.jpg')
    st.image(image, caption='SHABBIR....')
